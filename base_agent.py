...
class BaseAgent(object):
    ...
    def _handle_sigterm(self, *arg):
        ...
        # Delete Telemetry
        self._delete_telemetry(f".{self.name}")

        # Delete Notification Subscription
        self._unregister_all_notifications()
    ...
    def _unregister_all_notifications(self):
        """Unsubscribe the agent from all notifications"""
        request = NotificationRegisterRequest(
            stream_id=self.stream_id,
            op=NotificationRegisterRequest.Delete,
        )
        response = self.sdk_mgr_client.NotificationRegister(
            request=request, metadata=self.metadata
        )
    ...
    def _handle_notification(self, notification: Notification):
        ...
        """
        logger.debug(f"sub_id: {notification.sub_id}")

        if notification.HasField("config"):
            self._handle_ConfigNotification(notification.config)
        if notification.HasField("intf"):
            self._handle_InterfaceNotification(notification.intf)
        if notification.HasField("nw_inst"):
            self._handle_NetworkInstanceNotification(notification.nw_inst)
        if notification.HasField("lldp_neighbor"):
            self._handle_LldpNeighborNotification(notification.lldp_neighbor)
        if notification.HasField("bfd_session"):
            self._handle_BfdSessionNotification(notification.bfd_session)
        if notification.HasField("route"):
            self._handle_IpRouteNotification(notification.route)
        if notification.HasField("appid"):
            self._handle_AppIdentNotification(notification.appid)
        if notification.HasField("nhg"):
            self._handle_NextHopGroupNotification(notification.nhg)
